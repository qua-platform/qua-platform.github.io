...
def analog_output_port_to_pb(
    ...
    analog_output = qua_config.QuaConfigAnalogOutputPortDec(
        delay=int(data.get("delay", 0)), shareable=bool(data.get("shareable"))
    )
...
def digital_input_port_to_pb(data: Dict):
    digital_input = qua_config.QuaConfigDigitalInputPortDec(
        shareable=bool(data.get("shareable"))
    )
    ...
    if "polarity" in data:
        if data["polarity"].upper() == "RISING":
            digital_input.polarity = (
                qua_config.QuaConfigDigitalInputPortDecPolarity.RISING
            )
        elif data["polarity"].upper() == "FALLING":
            digital_input.polarity = (
                qua_config.QuaConfigDigitalInputPortDecPolarity.FALLING
            )
...
def controller_to_pb(data: Dict[str, Any]) -> qua_config.QuaConfigControllerDec:
    ...
    if "analog_outputs" in data:
        for _k, _v in data["analog_outputs"].items():
            cont.analog_outputs[int(_k)] = analog_output_port_to_pb(_v)
...
def oscillator_to_pb(
    ...
    if "intermediate_frequency" in data:
        if capabilities.supports_double_frequency:
            oscillator.intermediate_frequency_double = float(
...
def mixer_to_pb(data) -> qua_config.QuaConfigMixerDec:
    return qua_config.QuaConfigMixerDec(
        correction=[create_correction_entry(mixer) for mixer in data]
    )
...
def dac_port_ref_to_pb(
    controller: str, number: int
) -> qua_config.QuaConfigDacPortReference:
...
def adc_port_ref_to_pb(
    controller: str, number: int
) -> qua_config.QuaConfigAdcPortReference:
...
def digital_output_port_ref_to_pb(
    ...
    return qua_config.QuaConfigDigitalOutputPortReference(
        port=port_ref_to_pb(data[0], data[1])
    )
...
def element_to_pb(
    ...
    if "intermediate_frequency" in data:
        if capabilities.supports_double_frequency:
            element.intermediate_frequency_double = abs(
                float(data["intermediate_frequency"])
            )
            element.intermediate_frequency_oscillator_double = float(
                data["intermediate_frequency"]
            )
        else:
            element.intermediate_frequency = abs(int(data["intermediate_frequency"]))
            element.intermediate_frequency_oscillator = int(
                data["intermediate_frequency"]
            )
    ...
    if "mixInputs" in data:
        (cont_I, port_id_I) = data["mixInputs"]["I"]
        (cont_Q, port_id_Q) = data["mixInputs"]["Q"]
        element.mix_inputs = qua_config.QuaConfigMixInputs(
            ...
            mixer=data["mixInputs"].get("mixer", ""),
        )
        if capabilities.supports_double_frequency:
            element.mix_inputs.lo_frequency_double = float(
                data["mixInputs"].get("lo_frequency", 0.0)
            )
        else:
            element.mix_inputs.lo_frequency = int(
                data["mixInputs"].get("lo_frequency", 0)
            )

    if "singleInputCollection" in data:
        element.single_input_collection = qua_config.QuaConfigSingleInputCollection(
            inputs={
                k: dac_port_ref_to_pb(v[0], v[1])
    ...
    if "sticky" in data:
        if capabilities.supports_sticky_elements:
            ...
            element.hold_offset = qua_config.QuaConfigHoldOffset(
                duration=data["sticky"].get("duration", 1)
            )
        ...
        if capabilities.supports_sticky_elements:
            ...
            element.hold_offset = qua_config.QuaConfigHoldOffset(
                duration=data["hold_offset"]["duration"]
            )
    ...
    if "outputPulseParameters" in data:
        ...
        if signal_polarity == "ABOVE" or signal_polarity == "ASCENDING":
            output_pulse_parameters.signal_polarity = (
        ...
        if "derivativeThreshold" in pulse_parameters:
            output_pulse_parameters.derivative_threshold = pulse_parameters[
                "derivativeThreshold"
            ]
...
def waveform_to_pb(data) -> qua_config.QuaConfigWaveformDec:
    ...
    if data["type"] == "constant":
        ...
        wf.arbitrary = qua_config.QuaConfigArbitraryWaveformDec(
...
def digital_waveform_to_pb(data) -> qua_config.QuaConfigDigitalWaveformDec:
    return qua_config.QuaConfigDigitalWaveformDec(
        samples=[
            qua_config.QuaConfigDigitalWaveformSample(value=bool(s[0]), length=s[1])
...
def build_iw_sample(data) -> List[qua_config.QuaConfigIntegrationWeightSample]:
    if len(data) > 0 and not isinstance(data[0], tuple):
        ...
        data = new_data
    return [
        qua_config.QuaConfigIntegrationWeightSample(value=s[0], length=int(s[1]))
        for s in data
    ]
