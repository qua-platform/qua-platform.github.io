...
from qm.grpc.frontend import HaltRequest, ResumeRequest, ResumeResponse, PausedStatusResponse, PausedStatusRequest, \
    ...
    IsJobAcquiringDataRequest, JobExecutionStatus, GetJobExecutionStatusRequest, GetJobExecutionStatusResponse, \
    JobQueryParams, QueryValueMatcher, RemovePendingJobsResponse, GetPendingJobsResponse, JobExecutionStatusPending
from qm.grpc.general_messages import Matrix
from qm.grpc.job_manager import SetElementCorrectionRequest, GetElementCorrectionRequest, InsertInputStreamRequest, \
    IntStreamData, FixedStreamData, BoolStreamData
...
def test_set_element_correction(api, random_matrix):
    ...
    api._job_manager_stub.set_element_correction.assert_called_with(
        SetElementCorrectionRequest(
            job_id="test_job", qe_name="test_element", correction=Matrix(*random_matrix)
        ),
...
def test_get_element_correction(api, random_matrix):
    ...
    api._job_manager_stub.get_element_correction.assert_called_with(
        GetElementCorrectionRequest(
            job_id="test_job", qe_name="test_element"
        ),
        timeout=api._timeout
    )
    assert result == random_matrix
...
@pytest.mark.parametrize("data, data_type, data_object", [
...
def test_halt(api):
    ...
    api._frontend_stub.halt.assert_called_with(
        HaltRequest(job_id="test_job"),
        timeout=api._timeout
    )
...
def test_resume(api):
    ...
    api._frontend_stub.resume.assert_called_with(
        ResumeRequest(job_id="test_job"),
        timeout=api._timeout
    )
    assert result
...
def test_is_paused(api):
    ...
    api._frontend_stub.paused_status.assert_called_with(
        PausedStatusRequest(job_id="test_job"),
        timeout=api._timeout
    )
    assert result
...
def test_is_job_running(api):
    ...
    api._frontend_stub.is_job_running.assert_called_with(
        IsJobRunningRequest(job_id="test_job"),
        timeout=api._timeout
    )
    assert result
...
def test_is_data_acquiring(api):
    ...
    api._frontend_stub.is_job_acquiring_data.assert_called_with(
        IsJobAcquiringDataRequest(job_id="test_job"),
        timeout=api._timeout
    )
...
def test_get_job_execution_status(api):
    ...
    api._frontend_stub.get_job_execution_status.assert_called_with(
        GetJobExecutionStatusRequest(job_id="test_job", quantum_machine_id="machine_id"),
        timeout=api._timeout
    )
...
def test_remove_job(api):
    ...
    api._frontend_stub.remove_pending_jobs.return_value = RemovePendingJobsResponse(
        numbers_of_jobs_removed=1
    )
...
def test_get_running_job(api):
    api._frontend_stub.get_running_job.return_value = GetRunningJobResponse(
        job_id="test_job"
    )
    ...
    api._frontend_stub.get_running_job.assert_called_with(
        GetRunningJobRequest(machine_id="test_machine_id"),
        timeout=api._timeout
    )
