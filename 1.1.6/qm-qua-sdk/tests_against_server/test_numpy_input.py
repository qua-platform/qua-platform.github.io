...
def _run_sim(prog, host_port, vars_to_save):
    ...
    job = qmm.simulate(
        create_opx_config(),
        prog,
        SimulationConfig(duration=4000),
    )
...
def test_var_declare_numpy(host_port, float_type, int_type):
    ...
    with program() as prog:
        ...
        bool_arr = declare(bool, value=np.array([False, True, True, False, True], dtype=np.bool_))
...
def test_var_assign_numpy(host_port, float_type, int_type):
    ...
    assert res["b"][0]
    assert np.array_equal(res["c"], [float_to_fix(float_type(0.85)), float_to_fix(float_type(-2.2))])
...
def test_for_loop_numpy(host_port, float_type, int_type):
    with program() as prog:
        ...
        with for_(iter_f, init=int_type(0), cond=iter_f < float_type(5), update=iter_f + float_type(0.5)):
            save(iter_f, "iter_f")
        iter_int = declare(int, value=int_type(0))
        with for_(iter_int, init=int_type(0), cond=iter_int < int_type(5), update=iter_int + int_type(1)):
        ...
        for method in math_methods:  # math_methods:
            ...
            with program() as p:
                ...
                if method in math_methods_single_scalar + math_methods_single_scalar_int_return:
                    ...
                    assign(expected_ret[0], call_method(declare(fixed, value=1.5), declare(fixed, value=2)))
